              ...
              <div key={i} className="relative group">
                ...
                <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-30 transition-all duration-200 rounded-lg flex items-center justify-center">
                  <button
                    type="button"
                    onClick={() => onRemove(i)}
                    className="opacity-0 group-hover:opacity-100 transform scale-50 group-hover:scale-100 transition-all duration-200 bg-red-500 text-white p-1.5 rounded-full hover:bg-red-600"
                  >
                    <X size={14} />
                  </button>
                </div>
...
export default function AddBlogPage() {
  ...
  const handleFileUpload = async (urls) => {
    setBlog((prev) => ({ ...prev, images: [...prev.images, ...urls] }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let allblog = blog;
      if (!blog.slug) {
        const newslug = `${blog.title.split(" ").join("-")}-${Date.now()}`;
        allblog = { ...allblog, slug: newslug };
      }
